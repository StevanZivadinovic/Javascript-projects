...
topFifty.forEach((element) => {
  ...
  table.innerHTML += `<tr>
    ...
    <td class='${
      element.quote.USD.percent_change_24h.toFixed(2) > 0
        ? "positive"
        : "negative"
    }' >${element.quote.USD.percent_change_24h.toFixed(2)}%</td>
    <td><form class='inputField'><input pattern='[0-9]+' onkeyup='enable()' value='${
      localStorage.getItem(element.symbol) &&
      localStorage.getItem(element.symbol) !== null
        ? JSON.parse(localStorage.getItem(element.symbol))[0]
        : ""
    }'  class='amount' type='text'><button class='dugmici' disabled='true'>Submit</button></form></td>
    <td>${
      localStorage.getItem(element.symbol) &&
      localStorage.getItem(element.symbol) !== null
        ? JSON.parse(localStorage.getItem(element.symbol))[1]
        : ""
    }</td>
    </tr>`;
...
document.querySelectorAll(".inputField").forEach((a) => {
  a.addEventListener("submit", (e) => {
    ...
    let vrednost =
      parseInt(a.firstChild.value) *
      parseInt(
        a.parentElement.previousElementSibling.previousElementSibling.innerText
      );
    a.parentElement.nextElementSibling.innerText = `$${vrednost}`;
    localStorage.setItem(
      nameOfCurrency,
      JSON.stringify([parseInt(a.firstChild.value), vrednost])
    );
  });
});
...
document.querySelectorAll(".link").forEach((a) => {
  a.addEventListener("click", (e) => {
    ...
    localStorage.setItem(
      "dataOfCriptocurency",
      JSON.stringify(dataOfCriptocurency)
    );
